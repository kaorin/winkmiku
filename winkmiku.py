...
<interface>
  ...
  <object class="GtkWindow" id="WinkMiku">
    ...
    <signal name="size-allocate" handler="on_WinkMiku_size_allocate" swapped="no"/>
    <child>
      <placeholder/>
    </child>
    <child>
      <object class="GtkImage" id="miku">
...
class Miku:
    ...
    def __init__(self):
        ...
        self.image = self.wTree.get_object ("miku")
        ...
        dic = {
            ...
            "on_WinkMiku_size_allocate" : self.on_WinkMiku_size_allocate,
        ...
        self.inFocus = False
        self.image.set_from_file(os.path.dirname(os.path.abspath(__file__))+"/miku-a.png")
        self.icon = 0
        ...
        h = self.image.get_pixbuf().get_height()
        w = self.image.get_pixbuf().get_width()
    ...
    def on_WinkMiku_size_allocate(self, win, allocation):
        print ("on_WinkMiku_size_allocate")
        # オフスクリーンのPixmapを作成する。
        # マスクに使いたいだけなので、色深度は１（白黒）に。
        self.inFocus = False
        bitmap = GdkPixbuf.Pixbuf.new(GdkPixbuf.Colorspace.RGB,False,8,
                                allocation.width,
                                allocation.height)

        #cairoオブジェクト生成
        ca = self.image.get_window().cairo_create()
        ...
        if self.icon == 0:
            img = cairo.ImageSurface.create_from_png(os.path.dirname(os.path.abspath(__file__))+"/miku-b.png")
        else:
            img = cairo.ImageSurface.create_from_png(os.path.dirname(os.path.abspath(__file__))+"/miku-a.png")
        ca.set_source_surface(img)
        ca.set_operator(cairo.OPERATOR_OVER)
        ca.paint()

        # ↑で作成したマスク用ピクセルマップを使ってWindowをマスク。
        win.shape_cobine_region(bitmap,
                               allocation.x,
                               allocation.y)
        return True
    ...
    def _changeIcon(self):
        ...
        if self.icon == 0:
            self.image.set_from_file(os.path.dirname(os.path.abspath(__file__))+"/miku-b.png")
        else:
            self.image.set_from_file(os.path.dirname(os.path.abspath(__file__))+"/miku-a.png")
